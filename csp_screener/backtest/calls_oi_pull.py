"""
AMENDMENT 12 DATA — single-name CALL OPEN-INTEREST pull (run AFTER calls_pull.py).
...
def pull_ticker(sym: str, intervals: list[list[str]]) -> dict:
    ...
    done_marker = tdir / ".complete_calls_oi"
    counts = {"oi_calls": 0, "skip": 0, "empty": 0, "fail": 0}
    ...
    for exp in sorted(set(exps)):
        ...
        for a, b in spans:
            ...
            if lo > hi:
                continue
            endpoints = [("oi_calls", "/v3/option/history/open_interest")]
...
def main() -> int:
    ...
    log = open(DATA / "calls_oi_pull.log", "a", encoding="utf-8", buffering=1)
    print(f"{len(work)} member tickers to pull (oi={'on' if PULL_OI else 'OFF'})",
          file=log)
    totals = {"oi_calls": 0, "skip": 0, "empty": 0, "fail": 0, "crashed": 0}
    t0 = time.time()
    done = 0
